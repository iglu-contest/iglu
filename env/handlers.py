...
class GridObservation(handlers.TranslationHandler):
    ...
    def __init__(self, grid_name, min_x, min_y, min_z, max_x, max_y, max_z):
        super().__init__(space=spaces.Box(
            low=0, high=6, shape=(BUILD_ZONE_SIZE_Y, 
                                  BUILD_ZONE_SIZE_X, 
                                  BUILD_ZONE_SIZE_Z)
            )
        )
    ...
    def from_hero(self, x):
        ...
        return blocks_id.reshape(BUILD_ZONE_SIZE_Y, BUILD_ZONE_SIZE_X, BUILD_ZONE_SIZE_Z)
...
class GridIntersectionMonitor(handlers.TranslationHandler):
    def __init__(
        ...
        self.current_task = 'blue5L'
        self.task = Task()
        self.task_list = list(tasks.keys())
        self.task.load(tasks[self.current_task])
        self.prev_grid_size = 0
        self.max_int = 0
        super().__init__(space=spaces.Box(low=-1, high=1, shape=()))

    def reset(self):
        self.current_task = self.task_list[np.random.choice(len(tasks))]
        self.task.load(tasks[self.current_task])
        self.prev_grid_size = 0
        self.max_int = 0
    ...
    def from_hero(self, x):
        ...
        blocks_id = np.array([block2id.get(block, -1) for block in blocks])
        grid = blocks_id.reshape(BUILD_ZONE_SIZE_Y, BUILD_ZONE_SIZE_X, BUILD_ZONE_SIZE_Z)
        grid_size = (grid != 0).sum().item()
        wrong_placement = (self.prev_grid_size - grid_size) * self.wrong_scale
        max_int = self.task.maximal_intersection(grid) if wrong_placement != 0 else self.max_int
